...
In the cleaner version of the data for which only squirrels with non-missing data have been kept there are data for `r df_washing_squirrel |> select (unique_squirrel_id) |> distinct() |> count()` distinct squirrels. The data was collected between `r max(df_washing_squirrel |>  pull(date))` and `r min(df_washing_squirrel |>  pull(date))`. 
